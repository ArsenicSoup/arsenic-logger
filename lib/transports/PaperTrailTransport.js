...
var PaperTrailTransport = {
    ...
    */
    ...
    __getText : function(args, level, stackObj, hasTag, settings, filterOptions){
        ...
        var col = "";
        var reset = parseInt('033',8) + PaperTrailTransport.colors.reset;

        switch(level){
            case 'log': col = parseInt('033',8) + PaperTrailTransport.colors.green; log_level = 0; break;
            case 'debug': col = parseInt('033',8) + PaperTrailTransport.colors.green; log_level = 1; break;
            case 'info': col = parseInt('033',8) + PaperTrailTransport.colors.blue; log_level = 2; break;
            case 'warn': col = parseInt('033',8) + PaperTrailTransport.colors.magenta; log_level = 3; break;
            case 'error': col = parseInt('033',8) + PaperTrailTransport.colors.lightRed; log_level = 4; break;
            case 'fatal': col = parseInt('033',8) + PaperTrailTransport.colors.red; log_level = 5; break;
            case 'exception': col = parseInt('033',8) + PaperTrailTransport.colors.whiteOnRed; log_level = 5; break;
        }

        var trace_str = parseInt('033',8) + PaperTrailTransport.colors.gray;
        var depth = 1;
        var startDepth = 3;
